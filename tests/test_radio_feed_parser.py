...
class TestFetchDramaProgrammes:
    """Tests for fetch_drama_programmes URL construction."""

    @patch("radio_cache.bbc_feed_parser._fetch_json")
    def test_uses_playable_endpoint(self, mock_fetch: object) -> None:
        """Calls the /v2/programmes/playable endpoint, not inline/categories."""
        mock_fetch.return_value = {"data": [], "total": 0}  # type: ignore[attr-defined]
        fetch_drama_programmes(category_slugs=["drama"], max_pages=1, delay=0)
        url = mock_fetch.call_args[0][0]  # type: ignore[index]
        assert "/v2/programmes/playable?" in url
        assert "category=drama" in url
        assert "inline/categories" not in url

    @patch("radio_cache.bbc_feed_parser._fetch_json")
    def test_pagination_uses_offset(self, mock_fetch: object) -> None:
        ...
        urls = [c[0][0] for c in mock_fetch.call_args_list]  # type: ignore[index]
        assert f"offset=0" in urls[0]
        assert f"offset={_PAGE_LIMIT}" in urls[1]

    @patch("radio_cache.bbc_feed_parser._fetch_json")
    def test_includes_tleo_distinct(self, mock_fetch: object) -> None:
        """Request includes tleoDistinct=true for de-duplication."""
        mock_fetch.return_value = {"data": [], "total": 0}  # type: ignore[attr-defined]
        fetch_drama_programmes(category_slugs=["drama"], max_pages=1, delay=0)
        url = mock_fetch.call_args[0][0]  # type: ignore[index]
    ...
    @patch("radio_cache.bbc_feed_parser._fetch_json")
    def test_deduplicates_across_categories(self, mock_fetch: object) -> None:
        """Programmes appearing in multiple categories are deduplicated."""
        item = {"id": "b09dup", "title": "Shared Drama"}
        mock_fetch.return_value = {"data": [item], "total": 1}  # type: ignore[attr-defined]
