...
- `ifa/models.py`: typed data models (events, pensions, results).
...
3. Generate market returns (single path or many Monte Carlo paths).
4. Simulate pot balances over time.
5. Summarize results and render charts.

```mermaid
flowchart LR
    A[Inputs: ages, pots, spending, events] --> B[build_required_withdrawals]
    B --> C[withdrawals_required by age]
    C --> D[simulate_multi_pot_pension_path]
    D --> E[Path balances]
    E --> F[summarize_path]
    E --> G[plot_* charts]
