...
- **URL-based presets** (`ifa/url_presets.py`) let users share presets as shareable
    links without needing a backend server:
  - Presets are encoded as compressed base64 query parameters.
  - The `Generate URL` button creates a shareable link from current parameters.
