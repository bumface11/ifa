# Architecture

This project is organized so each part does one job.

## High-Level Structure

- `ifa_web.py`: Streamlit user interface.
- `pension_drawdown_simulator.py`: command-line runner that saves charts.
- `ifa/config.py`: default assumptions.
- `ifa/models.py`: typed data models (events, DB/DC pensions, results).
- `ifa/events.py`: converts spending + life events into yearly withdrawals.
- `ifa/engine.py`: core simulation logic.
- `ifa/market.py`: return sequence generation.
- `ifa/metrics.py`: summary statistics.
- `ifa/explain.py`: plain-English explanation text.
- `ifa/plotting.py`: matplotlib chart builders.
- `ifa/presets.py`: *Deprecated* - local save/load/delete helpers for Streamlit
    sidebar presets. Kept for backward compatibility.
- `ifa/url_presets.py`: URL-based preset encoding/decoding using compressed
    base64-encoded query parameters. Enables shareable preset links.

```mermaid
flowchart TD
    UI[ifa_web.py Streamlit UI] --> Events[ifa.events]
    UI --> Engine[ifa.engine]
    UI --> Plotting[ifa.plotting]
    UI --> Metrics[ifa.metrics]
    UI --> Explain[ifa.explain]
    UI --> URLPresets[ifa.url_presets]

    CLI[pension_drawdown_simulator.py CLI] --> Events
    CLI --> Engine
    CLI --> Plotting
    CLI --> Metrics

    Config[ifa.config] --> UI
    Config --> CLI
    Models[ifa.models] --> Events
    Models --> Engine
    Models --> Explain
```

## Data Flow (Simulation)

The same simulation pipeline is used by both CLI and Streamlit.

1. Build ages and DB income by year.
2. Build required withdrawals from baseline spending and life events.
3. Build DC pot list with per-pot drawdown start ages.
4. Generate market returns (single path or many Monte Carlo paths).
5. Simulate pot balances over time.
6. Summarize results and render charts.
7. Encode/decode sidebar parameters via shareable URLs (ifa/url_presets.py).
8. URL-based presets auto-load when a link is shared; if unsaved changes exist,
    UI warns before overwriting current inputs.

```mermaid
flowchart LR
    A[Inputs: ages, DC pots, DB incomes, spending, events] --> B[build_required_withdrawals]
    B --> C[withdrawals_required by age]
    C --> D[simulate_multi_pot_pension_path with dc_pots]
    D --> E[Path balances]
    E --> F[summarize_path]
    E --> G[plot_* charts]
    A --> K[encode_preset_url / decode_preset_url]

    H[run_monte_carlo_simulation] --> I[Monte Carlo paths]
    I --> J[summarize_monte_carlo]
    I --> G
```

## Why This Design Is Beginner-Friendly

- You can change UI or CLI behavior without touching core engine logic.
- DC pot drawdown ages are configured in one place and reused by both frontends.
- Life-event math is centralized in `ifa/events.py`.
- Chart functions read simulation outputs and do not own business rules.
- Explanations are generated by `ifa/explain.py`, separate from plotting.
- **URL-based presets** (`ifa/url_presets.py`) automatically update the browser URL
    as parameters change, enabling seamless sharing:
  - The URL encodes all parameters as a compressed base64 query string.
  - Users click `Share Link` to display the current URL with a copy button.
  - Visiting a shared link auto-loads parameters without user intervention.
  - All encode/decode logic is testable and independent of Streamlit.
